...
云上叠加两层随降水覆盖生成的轻雾，以 7 / 11 秒不同周期飘散和淡入淡出；仅动画变换与透明度，不逐帧重绘云团。雾气为装饰动效，不代表预报风向；切换原图或隐藏页面时暂停。默认遵循系统减少动态效果设置，可通过「雾气：静止 / 流动」手动切换。
长雨带 GeoJSON 按 JMA 时次表请求；若公布时次的资源返回 404 或无效数据，则显示暂无法判断，不会误报无长雨带。同一资源的结果在当前会话缓存（最多 8 个），避免重复探测多个地址。
